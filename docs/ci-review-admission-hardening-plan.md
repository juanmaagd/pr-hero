...
- [ ] 00.1 Define `ci_review_policy` values and defaults.
- [ ] 00.2 Define whether limits apply per PR, per head, or both.
- [ ] 00.3 Define `ci_max_attempts`, reservation TTL, and policy version/hash.
- [ ] 00.4 Define every admission reason and user-facing message.
- [ ] 00.5 Define `manual-required` as distinct from `skip`.
- [ ] 00.6 Decide whether `every_push` is allowed to bypass only score admission or also the automatic attempt budget. It must not bypass the budget.
...
- [ ] 01.1 Use `doc.findings` as the canonical current-run source.
- [ ] 01.2 Remove the `doc.findings + outside` double count at `src/cli.ts:3084`.
- [ ] 01.3 Deduplicate historical posted findings by `headSha + path + line + claimFingerprint`.
- [ ] 01.4 Ensure the same finding posted inline and as an issue comment counts once.
- [ ] 01.5 Never use severity headline counts for admission.
- [ ] 01.6 Represent unreadable historical data as unknown, not score zero.

Tests:

- [ ] Two Outside-Diff findings produce two findings, not four.
- [ ] Duplicate markers do not inflate score.
- [ ] Tier and severity remain independent.
- [ ] Malformed marker data cannot become a valid zero score.
...
- [ ] 02.1 Implement `reserveAdmissionAttempt()`.
- [ ] 02.2 Reserve immediately before provider launch.
- [ ] 02.3 Increment the attempt number during reservation.
- [ ] 02.4 Implement `settleAdmissionAttempt()` for every terminal path.
- [ ] 02.5 Persist failed, cancelled, and unknown outcomes.
- [ ] 02.6 Add reservation expiration and stale-reservation handling.
- [ ] 02.7 Make reservation idempotent with `PR + head SHA + policy hash`.
- [ ] 02.8 Store workflow run ID and provider-start timestamp.
- [ ] 02.9 Add required GitHub permissions and verify them in CI setup tests.
...
- [ ] 03.2 Bind authority to exact PR number, head SHA, policy hash, and schema version.
- [ ] 03.3 If historical comments are supported, require the trusted bot author.
- [ ] 03.4 Ignore contributor-authored lookalike markers.
- [ ] 03.5 Reject mismatched summary/state/admission heads.
- [ ] 03.6 Define safe migration for legacy headless comments.
- [ ] 03.7 Treat ambiguous or malformed historical authority as unknown.
...
- [ ] 04.1 Compute the delta from the last authoritative reviewed head to the current head.
- [ ] 04.2 Retrieve changed paths through a cheap GitHub API call before provider spend.
- [ ] 04.3 Classify high-risk and low-risk paths.
- [ ] 04.4 Version the risk policy.
- [ ] 04.5 Include `riskClass` and `riskReason` in the admission record.
- [ ] 04.6 Treat failed or empty path metadata as unknown, never safe.
...
- [ ] 05.1 Keep admission before worktree creation and provider launch.
- [ ] 05.2 Ensure no provider starts without a successful reservation.
- [ ] 05.3 Settle records from success, error, timeout, signal, and cancellation paths.
- [ ] 05.4 Keep `--force` as an explicit manual override.
- [ ] 05.5 Ensure skip output says “not reviewed”, never “clean”.
- [ ] 05.6 Ensure `manual-required` points to the exact override procedure.
...
- [ ] 06.1 Add cursors to the GraphQL issue-comment query.
- [ ] 06.2 Add cursors to the GraphQL review-thread query.
- [ ] 06.3 Iterate until `hasNextPage` is false.
- [ ] 06.4 Fail loudly when `hasNextPage` is true without `endCursor`.
- [ ] 06.5 Add a hard maximum page bound.
- [ ] 06.6 Represent truncation as unknown, not an empty comment set.
...
- [ ] First review runs.
- [ ] Same head skips.
- [ ] Documentation-only delta skips under `risk_aware`.
- [ ] Production delta runs after a low-score review.
- [ ] Threshold score triggers re-review.
- [ ] Outside-Diff findings count once.
- [ ] Untrusted marker is ignored.
- [ ] Mismatched head becomes unknown/manual.
- [ ] Failed provider consumes an attempt.
- [ ] Cancelled provider consumes an attempt.
- [ ] Unknown attempt does not restart forever.
- [ ] Exhausted budget produces `manual-required`.
- [ ] Concurrent reservation is idempotent.
- [ ] GraphQL pagination finds older markers.
- [ ] `--force` works only as an explicit override.
...
- [ ] 08.1 Add decision, reason, current head, reviewed head, risk, score, attempt, and remaining budget to the step summary.
- [ ] 08.2 Add distinct `manual-required` output.
- [ ] 08.3 Document all policy configuration keys and modes.
- [ ] 08.4 Document trusted configuration ownership.
- [ ] 08.5 Document the manual override procedure.
...
- [ ] Forged PR comments cannot suppress reviews.
- [ ] Risky production commits are not skipped solely because the prior score was low.
- [ ] Failed and cancelled provider runs consume attempts.
- [ ] Repeated pushes cannot launch unlimited reviews.
- [ ] Outside-Diff findings are not double-counted.
- [ ] GraphQL pagination is complete.
- [ ] Unknown state is visible and actionable.
- [ ] Integration tests cover the full CLI path.
- [ ] Full test suite passes.
- [ ] Typecheck passes.
- [ ] Biome passes without new warnings.
- [ ] Documentation explains policies and overrides.
