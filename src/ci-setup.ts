...
jobs:
  ...
  review:
    ...
    steps:
      ...
      #
      # KNOWN GAP, stated here so this comment does not overclaim: the two
      # outcomes that publish NO outputs — every hunter died, or a fatal
      # error — leave \`run-dir\` empty, so the emptiness check below suppresses
      # the upload for exactly the case \`always()\` exists to cover. Closing it
      # needs the action to publish \`run-dir\` on the failure path; until then
      # \`always()\` covers the rest (a review that ran but could not post, a
      # later step failing, a cancelled job).
      #
