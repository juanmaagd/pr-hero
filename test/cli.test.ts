// Caller-level tests for cli.ts's ROADMAP B6 wiring (WU6/PR3): runPostCommand
// (the $0 gate before the first live GitHub write), runTriageCommand/
// runTriageReplyCommand, runDoctorCommand's config-load resilience, and
// main()'s fatal-exit wiring. Same fake-gh pattern as test/pr/pr.test.ts,
// extended to capture stdin (needed to tell a leftover W1 finding issue
// comment from the summary comment — both hit the same
...
// src/pr/pr.ts and src/pr/inline.ts (cli-decomp S2, Cluster C); this file's
// makeFakeGh/doc/finding fixture block below is duplicated there rather than
// shared, per this file's own precedent (see the fixture block's own header).
